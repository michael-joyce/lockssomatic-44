...
class PluginControllerTest extends BaseTestCase {
    ...
    public function testAdminEdit() {
        ...
        $form = $formCrawler->selectButton('Update')->form([
                // DO STUFF HERE.
                // 'plugins[FIELDNAME]' => 'FIELDVALUE',
        ]);

        $client->submit($form);
        $this->assertTrue($client->getResponse()->isRedirect('/plugin/1'));
    ...
    public function testAdminDelete() {
        ...
        $preCount = count($em->getRepository(Plugin::class)->findAll());
        $client = $this->makeClient([
            'username' => 'admin@example.com',
            'password' => 'supersecret',
        ]);
        ...
        $em->clear();
        $postCount = count($em->getRepository(Plugin::class)->findAll());
